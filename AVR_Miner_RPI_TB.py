...
def load_config():
    ...
    if not Path(str(Settings.DATA_DIR) + '/Settings.cfg').is_file():
        ...
        while not correct_username:
            ...
            #jk r = requests.get(f"https://server.duinocoin.com/users/{username}", 
            #jk                  timeout=Settings.SOC_TIMEOUT).json()
            #jk correct_username = r["success"]
            #jk if not correct_username:
            #jk     print(get_string("incorrect_username"))
...
def mine_avr(com, threadid, fastest_pool):
    ...
    while True:
        ...
        while True:
            try:
                if retry_counter > 3:
                    #jk fastest_pool = Client.fetch_pool()
                    fastest_pool = None
                    retry_counter = 0

                debug_output(f'Connecting to {fastest_pool}')
                #jk s = Client.connect(fastest_pool)
                #jk server_version = Client.recv(s, 6)
                server_version = 1
                ...
                if threadid == 0:
                    ...
                    #jk Client.send(s, "MOTD")
                    #jk motd = Client.recv(s, 1024)
                    motd = "Mocked server\n"
        ...
        while True:
            try:
                ...
                #jk Client.send(s, job_request)
                #jk job = Client.recv(s, 128).split(Settings.SEPARATOR)
                ...
                idiff = "10"
                exp_result = 50
                job = [prev_hash, exp_hash, idiff]
                job = ["f96a54b0ec35bebc95f4d7a8f4aefecab3a3778c","985ba4d68b8954e8a9d010b7a7a7ca42ba57b24d","8"]
                exp_result = 242
            ...
            try:
                #jk Client.send(s, str(num_res)
                #jk             + Settings.SEPARATOR
                #jk             + str(hashrate_t)
                #jk             + Settings.SEPARATOR
                #jk             + f'RPI I2C AVR Miner {Settings.VER}'
                #jk             + Settings.SEPARATOR
                #jk             + str(rig_identifier)
                #jk             #+ str(port_num(com))
                #jk             + Settings.SEPARATOR
                #jk             + str(result[2]))

                responsetimetart = now()
                #jk feedback = Client.recv(s, 64).split(",")
                if num_res != exp_result:
                    feedback = ['BAD', "Unexpected result"]
                else :
                    feedback = ['GOOD']
                sleep(0.001)
...
if __name__ == '__main__':
    ...
    #jk try:
    #jk     check_mining_key(config)
    #jk except Exception as e:
    #jk     debug_output(f'Error checking miner key: {e}')
    #jk     
    #jk if donation_level > 0:
    #jk     try:
    #jk         Donate.load(donation_level)
    #jk         Donate.start(donation_level)
    #jk     except Exception as e:
    #jk         debug_output(f'Error launching donation thread: {e}')
    ...
    #jk if discord_presence == "y":
    #jk     try:
    #jk         init_rich_presence()
    #jk     except Exception as e:
    #jk         debug_output(f'Error launching Discord RPC thread: {e}')
